(ns learning.operation
  "OperationActor -- one learning operation = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor (LearningOps-
  LLM) is sealed into a single node (:advise); its proposal is ALWAYS
  routed through the Learner Safety Governor (:govern) and the
  ...
  One graph run = one learning operation (intake -> advise -> govern
  ...
  decision to a human operator (learning-coordinator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/finalize-support-plan`/`:actuation/contact-guardian`
  ALWAYS reach this node when the governor is clean -- see `learning.
  phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [learning.learningadvisor :as learningadvisor]
...
(defn build
  ...
  opts:
    :advisor      -- a `learning.learningadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (learningadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LearningOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (learningadvisor/-advise advisor store request)]
            {:proposal p :audit [(learningadvisor/trace request p)]})))

      ;; Learner Safety Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
